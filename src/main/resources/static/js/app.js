...
document.getElementById('combineFiles').addEventListener('change', function () {
  ...
  if (this.files.length > 0) {
    ...
    for (let i = 0; i < this.files.length; i++) {
      html += `<p>${i + 1}. ${this.files[i].name} (${(this.files[i].size / (1024
          * 1024)).toFixed(2)} MB)</p>`;
    }
...
// Compression slider
document.getElementById('compressionSlider').addEventListener('input',
    function () {
      document.getElementById('compressionValue').textContent = Math.round(
          this.value * 100) + '%';
    });
